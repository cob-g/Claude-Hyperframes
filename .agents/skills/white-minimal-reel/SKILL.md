...
4. **Get the objects.** Each scene needs one hero object that stands for its idea (a brain for
   ...
   ```sh
   node .agents/skills/white-minimal-reel/scripts/fetch-object.mjs "hourglass"
   node .agents/skills/white-minimal-reel/scripts/fetch-object.mjs "hourglass" --pick 2 \
...
6. **Build the compositions:**
   ...
   ```sh
   node .agents/skills/white-minimal-reel/scripts/build-reel.mjs video-projects/<slug>
   ```
...
3. Render a draft, then run the motion audit. It must report about 12 unique frames per second
   ...
   `node .agents/skills/white-minimal-reel/scripts/motion-audit.mjs video-projects/<slug> renders/<draft>.mp4`
